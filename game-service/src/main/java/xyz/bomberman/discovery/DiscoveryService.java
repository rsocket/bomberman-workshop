...
public class DiscoveryService extends BaseSubscriber<DataBuffer> implements DisposableBean {
  ...
  protected void hookOnNext(DataBuffer rawServiceEvent) {
    final ServiceEvent serviceEvent = ServiceEvent.getRootAsServiceEvent(rawServiceEvent.asByteBuffer());

    if (serviceEvent.eventType() == AnyServiceEvent.Connected) {
      final ServiceConnectedEvent event = (ServiceConnectedEvent) serviceEvent.event(new ServiceConnectedEvent());
